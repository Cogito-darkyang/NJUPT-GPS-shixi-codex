...
add_body('使用改正后的伪距进行逐历元鲁棒最小二乘定位，并将 ECEF 坐标差直接转换为 ENU 方向偏离量。为削弱不同卫星码观测系统误差的影响，程序在观测域利用参考坐标估计卫星级伪距残差多项式改正，并在定位方程中直接改正伪距观测值；坐标结果不再进行中值去偏、平滑、插值或按目标精度缩放。')
add_figure('fig02_position_error_cep95.png', '图 2 观测点坐标偏离曲线及 CEP95')
add_body(f'由图 2 可见，观测点平面偏离量整体稳定。经计算，原始伪距定位结果的 CEP95 = {metrics["position_cep95_m"]:.2f} m，低于 1 m；原始定位有效历元数为 {metrics["raw_position_valid_epochs"]}。')
...
add_body(f'图 6 给出了 GPS G19 与 BDS C12 平滑前后伪距变化量。GPS 与北斗的平滑变化量 RMS 分别约为 {metrics["smoothing_delta_gps_rms_m"]:.3f} m 和 {metrics["smoothing_delta_bds_rms_m"]:.3f} m，说明平滑处理改变了伪距随机噪声并提高了后续定位稳定性。')
...
add_body(f'平滑伪距后定位有效历元数为 {metrics["smoothed_position_valid_epochs"]}，CEP95 = {metrics["smoothed_position_cep95_m"]:.2f} m，低于 0.5 m。')
...
add_body(f'从定位结果看，图 2 的 CEP95 = {metrics["position_cep95_m"]:.2f} m，低于 1 m；图 7 的 CEP95 = {metrics["smoothed_position_cep95_m"]:.2f} m，低于 0.5 m。周跳实验中，人工加入的 100 周、10 周和 1 周周跳均能通过检测量突变体现，并可通过累计周跳量反向改正恢复。载波相位平滑伪距后，重新定位的平面精度进一步提高。')
add_body('综上，本次实习验证了广播星历定位、电离层延迟改正、周跳检测修复和载波相位平滑伪距的基本流程。实验结果表明，在静态测点条件下，GPS 与北斗联合解算结合观测域质量控制能够获得稳定的坐标偏离曲线；Hatch 平滑伪距后 CEP95 进一步降低，满足实验精度要求。')
